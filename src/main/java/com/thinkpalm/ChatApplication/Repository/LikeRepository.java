...
public interface LikeRepository extends JpaRepository<LikeModel,Integer> {
    ...
    @Query(value = "select u.name,u.profilePic,lm.liked_at from like_message as lm inner join user as u on lm.user_id = u.id where lm.message_id = ?",nativeQuery = true)
