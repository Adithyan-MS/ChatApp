...
//    List<Map<String, Object>> getAllUserChatMessages(Integer currentUserId,Integer otherUserId);
    ...
    //new
    @Query(value = "SELECT m.id, m.content, m.sender_id, u.name as sender_name,case when sm.id is null then 0 else 1 end as is_starred, m.parent_message_id,u1.name as parent_message_sender ,m1.content as parent_message_content, m.like_count, m.created_at, m.modified_at\n" +
